(() => {
  ...
  // ns-params:@params
  var whisperWorkerPath = "/js/whisper.worker.js";

  // ns-hugo:/home/runner/work/whisper.aidanspendlove.dev/whisper.aidanspendlove.dev/assets/js/utils.js
  var MessageTypes = {
  ...
  var GENERATED_TEXT_CLASS = "generated_text";
  var START_TIME_ATTR = "data-timestamp-start";
  var END_TIME_ATTR = "data-timestamp-end";
  ...
  var PARTIAL_RESULTS_CONTAINER = document.getElementById(
    "partial-results-container"
  );
  var TRANSCRIPT_CONTAINER = document.getElementById("transcription-container");
  var DOWNLOAD_TRANSCRIPT_BTN = document.getElementById(
    "download-results-as-csv-btn"
  );
  var CLOSE_TRANSCRIPT_BTN = document.getElementById(
    "close-transcription-container-btn"
  );
  var PROGRESS_BAR = document.getElementById("transcription-progress-bar");
  var PROGRESS_BAR_CONTAINER = document.getElementById(
    "transcription-progress-bar-container"
  );
  var MODEL_NAME_DISPLAY = document.getElementById("model-name-display");
  var LOADING_SPINNER_CONTAINER = document.getElementById(
    "loading-spinner-container"
  );
  var LOADING_MESSAGE_CONTAINER = document.getElementById(
  ...
  (0, import_jquery.default)(document).ready(() => {
    ...
    FILE_UPLOAD_BTN.addEventListener("change", (event2) => onFormInputChanges());
    MODEL_NAME_SELECTION_INPUT.addEventListener(
      "change",
      (event2) => onFormInputChanges()
    );
    VIDEO_PLAYER.addEventListener(
      "timeupdate",
      (event2) => highlightCurrentChunk(event2.target.currentTime)
    );
    CLOSE_TRANSCRIPT_BTN.addEventListener("click", (event2) => {
      event2.preventDefault();
      showFormView();
    });
    DOWNLOAD_TRANSCRIPT_BTN.disabled = true;
    DOWNLOAD_TRANSCRIPT_BTN.addEventListener("click", (event2) => {
      event2.preventDefault();
      downloadTranscript();
    });
    WORKER = createWorker();
  ...
  function createWorker() {
    ...
    worker.onmessage = (event2) => {
      ...
      if (type === MessageTypes.RESULT_PARTIAL) {
        handlePartialResultMessage(event2.data);
      }
  ...
  function highlightCurrentChunk(currentTime) {
    ...
    for (let i = 0; i < spans.length; i++) {
      const span = spans[i];
      const start2 = parseFloat(span.getAttribute(START_TIME_ATTR));
      const end2 = parseFloat(span.getAttribute(END_TIME_ATTR));
  ...
  function handleLoadingMessage(data) {
    ...
    if (status === LoadingStatus.SUCCESS) {
      LOADING_MESSAGE_CONTAINER.innerHTML = "Model loaded successfully. Starting transcription...";
    }
    if (status === LoadingStatus.ERROR) {
      LOADING_MESSAGE_CONTAINER.innerHTML = "Oops! Something went wrong. Please refresh the page and try again.";
    }
  ...
  function handleResultMessage(data) {
    ...
    results.slice(0, RESULTS_CONTAINER.children.length).forEach((result, index) => {
  ...
  function hasResultChanged(result, element) {
    const start2 = parseInt(element.getAttribute(START_TIME_ATTR));
    const end2 = parseInt(element.getAttribute(END_TIME_ATTR));
    const text = element.innerText;
    return text !== result.text || start2 !== result.start || end2 !== result.end;
  }
  ...
  function createResultLine(result, isDone) {
    ...
    span.setAttribute(END_TIME_ATTR, `${end2}`);
    if (isDone) {
      span.setAttribute("data-bs-toggle", "tooltip");
      span.setAttribute("data-bs-placement", "top");
      span.setAttribute("title", `${start2} - ${end2}`);
      new Tooltip(span);
    }
    return span;
  }
  ...
  function downloadTranscript() {
    ...
    for (let i = 0; i < spans.length; i++) {
      ...
      data.push({ start: start2, end: end2, text });
    }
